import React from 'react';
import { experience } from '@/data/portfolio';

export default function Experience() {
  return (
    <section className="space-y-4 font-mono">
      <div className="flex items-center gap-2 text-xs tracking-widest text-zinc-700 dark:text-zinc-400 uppercase">
        <span>04</span>
        <span>—</span>
        <span>experience</span>
      </div>
      <div className="space-y-4">
        {experience.map((item, index) => (
          <div
            key={index}
            className="group flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 pb-3 border-b border-zinc-100 dark:border-white/5 last:border-0 last:pb-0 transition-colors"
          >
            <div>
              <h3 className="text-xs md:text-sm font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight uppercase">
                {item.role}
              </h3>
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5 font-light">
                {item.company}
              </p>
            </div>
            <span className="shrink-0 text-[11px] text-zinc-400 dark:text-zinc-500">
              {item.year}
            </span>
          </div>
        ))}
      </div>
    </section>
  );
}